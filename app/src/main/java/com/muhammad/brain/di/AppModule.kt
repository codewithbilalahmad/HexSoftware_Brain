...
import com.muhammad.brain.data.settings.SettingPreferencesImp
...
val appModule = module {
    single { BrainApplication.INSTANCE }
    single { HttpClientFactory.createClient() }
    singleOf(::SettingPreferencesImp).bind<SettingPreferences>()
    singleOf(::QuizNetworkImp).bind<QuizNetwork>()
    singleOf(::QuizRepositoryImp).bind<QuizRepository>()
